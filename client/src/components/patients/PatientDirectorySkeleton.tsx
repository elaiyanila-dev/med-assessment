...
export const PatientDirectorySkeleton: React.FC = () => {
  return (
    <div className="space-y-6 animate-pulse p-6">
      ...
      {/* Filter Bar Skeleton */}
      <div className="flex flex-col lg:flex-row gap-4 items-center justify-between bg-slate-800/40 p-4 rounded-xl border border-slate-700/40">
        <div className="h-10 bg-slate-700 rounded-lg w-full lg:w-80"></div>
        <div className="flex gap-2 w-full lg:w-auto">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-8 bg-slate-700 rounded-lg w-24"></div>
          ))}
        </div>
      </div>
